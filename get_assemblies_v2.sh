...
function search_ncbi_and_filter ()
{
...
function download_file()
{
    output_path="$1"
    file_url="$2"
    wget -P "$output_path" "$file_url" #2> /dev/null
}

# Make functions available for parallel
export -f search_ncbi_and_filter
export -f search_ncbi
export -f download_file



# If specified assembly levels
if [ "$assembly_level" != "all" ]; then  #Get only selected assembly levels
    # If not list
    if [ -z "$list" ]; then
        echo "Performing eSearch for specified assembly level(s)"
        search_ncbi_and_filter "$query" \
            | parallel \
                --bar \
                --env out \
                --delay 1 \
                'download_file $out {} 2> /dev/null'
    else  # if list
        cat "$list" \
            | parallel \
                --delay 1 \
                --env search_ncbi_and_filter \
                --env db \
                --env assembly_level \
                --jobs "$cpu" \
                "download_file $out $(search_ncbi_and_filter {})"
            # | parallel \
            #     --bar \
            #     --env out \
            #     --delay 1 \
...
else  # Get all assembly levels
    if [ -z "$list" ]; then  # if no list
        echo "Performing eSearch for all assembly levels"
        search_ncbi "$query" \
            | parallel \
                --bar \
                --env out \
        ...
        for i in $(cat "$list"); do
            my_url=$(search_ncbi_and_filter "$i")
            download_file $out "$my_url"
        done
